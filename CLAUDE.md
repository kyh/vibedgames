...
- **Every PR gets Cloudflare Worker Previews, Vercel-style, driven by Actions rather than Workers Builds.** `.github/workflows/preview.yml` runs on `workflow_run` after CI is green (same-repo branches only, since that trigger holds secrets), deploys `wrangler preview --name pr-<n>` for web and party when the PR touches them, keeps one sticky status comment plus a GitHub deployment per Worker (`.github/scripts/worker-preview.mjs`), and deletes the previews when the PR closes. Previews inherit no bindings: each `wrangler.jsonc` restates them in its `previews` block against `vibedgames-preview` (D1) and `vibedgames-games-preview` (R2), which the workflow creates on first use and swaps in for `<REPLACE_ME>` before building. PR code never reaches prod D1 or the prod bucket. The games Worker has no preview because it routes by `{slug}.vibedgames.com`. Preview secrets live in Cloudflare's preview base config (`wrangler preview base-config secret put`), not in GitHub.
