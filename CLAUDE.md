...
- **better-auth is pinned to `~1.6.29`** (tilde, so a fresh resolve can't float into 1.7). 1.7 makes `account.issuer` a required column and adds a unique `(issuer, accountId)` index; `packages/db/src/drizzle-schema-auth.ts` has neither, and nothing in `pnpm verify` touches D1, so the break only shows up as a failed sign-in against the live database. Adopting 1.7 needs a schema regen, a `db:push` and an identity backfill — and `@better-auth/cli` is stuck at 1.4.21, so `generate:auth-schema` can't emit `issuer` and strips the hand-curated index calls. Keep `@better-auth/api-key` and `@better-auth/expo` on the same 1.6 line, and keep `@better-auth/core` declared in `packages/api` — it is their peer, and left to auto-install it floats to 1.7 and puts a second copy of the table definitions in the bundle.
