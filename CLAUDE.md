...
- **Playtest decisions go through TypeSafe Jev (internal only).** `vg pilot` lets a model play a game: each tick the CLI reads `__GAME_DIAGNOSTICS__`, asks `pilot.decide` (`packages/api/src/pilot/`) which inputs to hold, and drives the browser. The server holds `TYPESAFE_API_KEY` and forwards exactly one System One request shape — state plus typed questions — so the key can't be used for anything else. Not metered: a run costs a fraction of a cent, and the per-call caps (64 KB state, 32 questions) bound it. Games advertise their controls in `window.__GAME_PILOT__`; `games/pong` is the reference.
...
- **Auth:** `VG_TOKEN` set as an environment secret (device-code `vg login` needs a browser and blocks an agent). `VG_API_URL` defaults to prod; override for local/staging. `vg pilot` needs only these — the decision model's key lives on the server.
