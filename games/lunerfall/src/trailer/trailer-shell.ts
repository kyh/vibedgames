...
const wait = (ms: number): Promise<void> =>
  // oxlint-disable-next-line promise/avoid-new -- setTimeout is a callback API; this is the promisified wrapper.
  new Promise((resolve) => {
...
export const runTrailer = (config: TrailerConfig): void => {
  ...
  const playScene = async (scene: TrailerScene, index: number, myGen: number): Promise<void> => {
    ...
    // oxlint-disable-next-line promise/avoid-new -- requestAnimationFrame is a callback API; the loop resolves when the scene ends.
