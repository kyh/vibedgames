import type Phaser from "phaser";
import { BlendModes } from "phaser";

import { COLORS } from "../config";
import { ROOM_LABEL } from "../data/rooms";
import type { RoomType } from "../data/rooms";
import type { Rect } from "./player-body";

const DANGER = new Set<RoomType>(["combat", "elite", "boss"]);

// Gate sprites cropped from the Luneblade prop sheet (env:props): a teal glowing
// doorway for safe paths, pink for danger. Registered once as named frames.
const registerGates = (scene: Phaser.Scene) => {
  const tex = scene.textures.get("env:props");
  if (tex.has("gate-teal")) {
    return;
  }
  tex.add("gate-teal", 0, 165, 23, 48, 51);
  tex.add("gate-pink", 0, 163, 101, 48, 51);
};

// An exit gate. Locked (dim) until the room is cleared, then it lights up and its
// trigger zone sends the player to the next room.
export class Door {
  private scene: Phaser.Scene;
  readonly x: number;
  readonly y: number;
  readonly type: RoomType;
  readonly index: number;
  private root: Phaser.GameObjects.Container;
  private glow: Phaser.GameObjects.Ellipse;
  private gate: Phaser.GameObjects.Image;
  private label: Phaser.GameObjects.Text;
  private color: number;
  active = false;

  constructor(scene: Phaser.Scene, x: number, y: number, type: RoomType, index: number) {
    this.scene = scene;
    this.x = x;
    this.y = y;
    this.type = type;
    this.index = index;
    registerGates(scene);
    const danger = DANGER.has(type);
    this.color = danger ? COLORS.magenta : COLORS.teal;
    this.root = scene.add.container(x, y).setDepth(15);
    this.glow = scene.add.ellipse(0, -24, 30, 50, this.color, 0.16).setBlendMode(BlendModes.ADD);
    this.gate = scene.add
      .image(0, 1, "env:props", danger ? "gate-pink" : "gate-teal")
      .setOrigin(0.5, 1);
    this.label = scene.add
      .text(0, -58, ROOM_LABEL[type], {
        color: "#f4f7fb",
        fontFamily: "monospace",
        fontSize: "7px",
      })
      .setOrigin(0.5, 1);
    this.root.add([this.glow, this.gate, this.label]);
    this.setActive(false);
  }

  setActive(v: boolean) {
    this.active = v;
    this.glow.setVisible(v);
    this.label.setAlpha(v ? 1 : 0.3);
    // Locked gates dim + desaturate toward the stone; cleared gates glow full.
    this.gate.setAlpha(v ? 1 : 0.42).setTint(v ? 0xff_ff_ff : 0x6f_7a_8c);
    this.scene.tweens.killTweensOf(this.glow);
    if (v) {
      this.glow.setScale(1);
      this.scene.tweens.add({
        alpha: 0.34,
        duration: 750,
        ease: "Sine.easeInOut",
        repeat: -1,
        scaleX: 1.35,
        targets: this.glow,
        yoyo: true,
      });
    }
  }

  triggerRect(): Rect {
    return { bottom: this.y, left: this.x - 12, right: this.x + 12, top: this.y - 42 };
  }

  destroy() {
    this.scene.tweens.killTweensOf(this.glow);
    this.root.destroy();
  }
}
