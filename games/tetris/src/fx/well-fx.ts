import { BoxGeometry, DoubleSide, Mesh, MeshBasicMaterial, RingGeometry, type Scene } from "three";
...
export class WellFx {
  ...
  private readonly notice = document.getElementById("fx-notice");
  private readonly warning = document.getElementById("height-warning");
  private cursor = 0;
  private dropAge = 1;
  ...
  constructor(scene: Scene) {
    ...
    this.marks = Array.from({ length: 64 }, () => ({ mesh: makeRing(0xffffff), age: 0, life: 0 }));
    const box = new BoxGeometry(0.08, 1, 0.08);
    this.streaks = Array.from({ length: 4 }, () => {
      const mesh = new Mesh(box, this.streakMaterial);
      mesh.visible = false;
      scene.add(mesh);
      return { mesh, length: 0, bottom: 0 };
    });
    this.pulseMesh = makeRing(0xffffff);
    this.dangerMesh = makeRing(0xffbd73);
  ...
  hardDrop(start: Cell[], landing: Cell[], color: number): void {
    ...
    for (let i = 0; i < this.streaks.length; i++) {
      ...
      if (to) streak.mesh.position.set(to.x, to.y + streak.length / 2, to.z);
  ...
  clear(cells: Cell[], lines: number, bothAxes: boolean): void {
    this.contact(cells, 0xffefc2, true);
    this.announce(
  ...
  power(cells: Cell[]): void {
    ...
    this.pulseMesh.material.color.set(0xffd86b);
    this.announce("POWER SWEEP", "#ffd86b");
  }

  rescue(): void {
    this.pulse = { kind: "rescue", age: 0, y: -0.42 };
    this.pulseMesh.material.color.set(0xa7e8dc);
