...
export const length = (p: Point): number => Math.sqrt(p.x * p.x + p.y * p.y);
...
export const randUniform = (max: number, min = 0): number => Math.random() * (max - min) + min;
 ...
 */
export const segmentsIntersect = (a1: Point, a2: Point, b1: Point, b2: Point): boolean => {
  const ax = a2.x - a1.x;
  const ay = a2.y - a1.y;
  const bx = b2.x - b1.x;
  const by = b2.y - b1.y;
  return (
    (ax * (b1.y - a1.y) - ay * (b1.x - a1.x)) * (ax * (b2.y - a1.y) - ay * (b2.x - a1.x)) <= 0 &&
    (bx * (a1.y - b1.y) - by * (a1.x - b1.x)) * (bx * (a2.y - b1.y) - by * (a2.x - b1.x)) <= 0
  );
};

/** Check if a point is within radius of another point (circle collision) */
export const circleContains = (center: Point, radius: number, p: Point): boolean => {
...
export const inWorld = (p: Point, margin = 0): boolean =>
  p.x >= -margin && p.x <= WORLD_WIDTH + margin && p.y >= -margin && p.y <= WORLD_HEIGHT + margin;
