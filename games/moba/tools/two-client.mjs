...
async function until(page, predicate, label, timeout = 20000, arg) {
...
const unitOf = (page, id) =>
  page.evaluate((id) => {
    ...
    return u ? { x: u.x, y: u.y, hp: u.hp, alive: u.alive, q: u.hero.abilities.Q } : null;
  }, id);
const seesUnit = (page, id, label) =>
  until(page, (id) => !!window.__moba.world.units.get(`h-${id}`), label, 20000, id);
const key = (page, type, key, keyCode, extra = {}) =>
  page.evaluate(
    ({ type, key, keyCode, extra }) =>
      window.dispatchEvent(
        new KeyboardEvent(type, { key, code: key, keyCode, bubbles: true, ...extra }),
      ),
    { type, key, keyCode, extra },
  );
...
async function promoted(client, follower) {
  await until(client.page, () => window.__moba.online().isHost, `${client.name} promoted`, 30000);
...
const run = async (base) => {
  ...
  try {
    ...
    await step("host transport drop: guest promoted, host reconnects as guest", async () => {
      ...
      await until(b.page, () => window.__moba.online().isHost, "guest promoted", 30000);
