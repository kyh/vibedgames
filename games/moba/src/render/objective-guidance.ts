...
export type StructureAnnouncement = {
  text: string;
  tone: "good" | "bad" | "neutral";
  priority: "objective" | "major" | "ending";
};

const TIER_DEPTH = { t1: 0, t2: 1, base: 2, ancient: 3 } satisfies Record<StructTier, number>;
...
function respawnTip(world: World, player: Unit): string | null {
  ...
  if (player.alive || !hero || hero.respawnAt <= 0) return null;
  ...
  if (upgrade) return `Unspent skill point: upgrade ${upgrade} before returning to lane.`;
  if (
...
export function objectiveGuidance(
  ...
  if (world.phase !== "playing" || !player?.hero || player.neutral) return null;
  ...
  for (const unit of world.units.values()) {
    ...
    if (
      ...
      continue;
    if (structure.tier === "base") baseTowers++;
  ...
  return { targetId: target.id, lane, text, respawnTip: respawnTip(world, player) };
}
...
export function structureAnnouncement(
  ...
  if (event.tier !== "ancient" && lane && lane !== "base") {
    ...
    if (event.tier === "t1") {
      ...
      if (exposed(world, event.team, "t2", lane)) text += " · INNER TOWER EXPOSED";
    } else if (event.tier === "t2") {
      text = `${position} LANE OPEN`;
      if (exposed(world, event.team, "base")) text += " · BASE TOWERS EXPOSED";
    }
