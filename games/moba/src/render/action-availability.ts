...
export function actionAvailability(u: Unit, now: number, action: Action): Availability {
  const h = u.hero;
  if (!h) return { kind: "blocked", reason: "unavailable" };
  if (!u.alive) return { kind: "blocked", reason: "dead" };
  if (disabled(u)) return { kind: "blocked", reason: "stunned" };
  if (action.kind === "dash")
  ...
  if (!def || slot.rank <= 0) return { kind: "blocked", reason: "unlearned" };
  if (def.targeting === "passive") return { kind: "blocked", reason: "passive" };
  if (silenced(u))
    return {
      kind: "blocked",
      reason: u.statuses.some((status) => status.kind === "stun") ? "stunned" : "silenced",
    };
  if (now < slot.readyAt) return { kind: "blocked", reason: "cooldown" };
  if (u.mp < valAt(def.manaCost, slot.rank)) return { kind: "blocked", reason: "mana" };
