...
const noiseBuffer = (c: AudioContext, dur: number): AudioBuffer => {
...
const noise = (dur: number, gain: number, lp: number): void => {
  const c = ac();
  if (!c || !master || muted) {
    return;
  }
...
const gate = (key: string, minMs: number): boolean => {
  const now = performance.now();
  if ((lastAt[key] ?? 0) + minMs > now) {
    return false;
  }
  lastAt[key] = now;
  return true;
...
export const sfx = {
  ...
  victory(win: boolean): void {
    ...
    for (const [i, f] of notes.entries()) {
      setTimeout(() => tone(f, 0.3, "triangle", 0.16), i * 160);
    }
  },
};
