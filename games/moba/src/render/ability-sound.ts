...
const PHRASES = {
  // Boomtinker — Q mechanical pin, W mine latch, E rocket lift, R the keg's heavy fuse.
  ...
  // Brewkeeper — Q a poured draught, W a wobbling hex, E a soft ward, R a resolving toast.
  "brewkeeper:E": [
  ...
  // Duskblade — Q a vanishing breath, W three quick blade edges, R an execution release.
  ...
  // Emberhex — Q fireball ignition, W a low fire ring, E a sharp flare, R a heavy conflagration.
  ...
  // Ironvow — Q buckler contact, W a held ward, R the low two-part vow.
  "ironvow:Q": [
  ...
  // Stormcaller — Q bow-string release, W a ringing mark, E rushing wind, R a storm downbeat.
...
export const abilityNotes = (effect: string): readonly AbilityNote[] =>
  byEffect.get(effect) ?? fallback;
...
export const abilityFoley = (effect: string) =>
  byEffect.has(effect) ? (accents.get(effect.split(":")[0] ?? "") ?? null) : null;
