...
export function acquireTarget(w: World, u: Unit): Unit | null {
  ...
  if (taunt) {
    ...
    if (t && targetable(t, { allowStructure: true })) {
      return t;
    }
  }
  ...
  if (u.kind === "structure") {
    return acquireForStructure(w, u, range);
  }
  ...
  for (const t of w.units.values()) {
    ...
    if (!targetable(t, { allowStructure: true })) {
      continue;
    }
    const d2 = dist2(u, t);
    if (d2 > range * range) {
...
function attackSpeedVsTarget(u: Unit, target: Unit): number {
  ...
  for (const s of u.statuses) {
    if (s.kind === "attackSpeed" && s.id === markId) {
      bonus += s.amount;
    }
  }
  return bonus;
...
export function resolvePendingAttacks(w: World): void {
  for (const u of w.units.values()) {
    ...
    if (dist(u, target) > u.attackRange + u.radius + target.radius + 40) {
      continue;
    }
...
function impactProjectile(w: World, p: Projectile, x: number, y: number): void {
  ...
  if (p.radius > 0) {
    ...
    for (const v of w.units.values()) {
      ...
      if (!v.alive || (!v.neutral && v.team === p.team)) {
        continue;
      }
      if (!targetable(v, { allowStructure: true })) {
        continue;
      }
      if (dist2(v, { x, y }) <= p.radius * p.radius) {
        applyTo(v);
      }
    }
  } else {
    const t = p.targetId ? w.units.get(p.targetId) : null;
    if (t && t.alive && (t.neutral || t.team !== p.team)) {
      applyTo(t);
    }
  }
}
