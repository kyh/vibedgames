...
export class BattleFx {
  ...
  burst(
    ...
    if (!this.visible(x, y, radius + 80)) return;
    let free = -1;
    let live = 0;
    let oldestCommon = -1;
    for (let i = 0; i < this.slots.length; i++) {
      ...
      if (b.importance === "common" && (!oldest || b.bornAt < oldest.bornAt)) oldestCommon = i;
    }
  ...
  private admitWeaponDraw(importance: FxImportance): boolean {
    ...
    if (this.weaponDraws >= budget) return false;
    this.weaponDraws++;
    return true;
  }
  ...
  beam(
    ...
    if (!this.admitWeaponDraw(importance)) return;
  ...
  orbit(
    ...
    if (!this.visible(x, y, radius + 40) || !this.admitWeaponDraw(importance)) return;
    const g = this.weaponGfx;
    const reduced = REDUCED_MOTION.matches;
  ...
  update(now: number): void {
    ...
    for (let i = 0; i < this.slots.length; i++) {
      ...
      if (b.kind === "fracture") this.drawFracture(b, glow, age, t, reduced);
      else if (b.kind === "impact" || b.kind === "muzzle")
        this.drawImpact(b, glow, age, t, reduced);
      else this.drawBlast(b, glow, age, t, reduced);
    }
  }
  ...
  private drawBlast(
    ...
    for (let stage = 0; stage < stages; stage++) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        g.lineStyle(i % 3 === 0 ? 2.4 : 1.3, i % 3 === 0 ? 0xfff2cb : blastTint, envelope * 0.95);
        g.lineBetween(
          x + Math.cos(a) * tail,
          y + Math.sin(a) * tail,
