...
for (const delay of [0, 140, 280]) assert.equal(burstStage(burstLifetime("boss"), delay, 1000), 0);
...
assert.deepEqual(encounters.observe(100, [seen("a")]), [{ kind: "arrival", id: "a", phase: 1 }]);
assert.deepEqual(encounters.observe(100, [seen("a", 661)]), []);
assert.deepEqual(encounters.observe(100, [seen("a", 660)]), [{ kind: "phase", id: "a", phase: 2 }]);
for (const hp of [660, 900, 500, 331])
  assert.deepEqual(encounters.observe(100, [seen("a", hp)]), []);
assert.deepEqual(encounters.observe(100, [seen("a", 330)]), [{ kind: "phase", id: "a", phase: 3 }]);
assert.deepEqual(
...
assert.deepEqual(encounters.observe(200, []), [{ kind: "defeat", id: "b" }]);
assert.deepEqual(encounters.observe(NaN, [seen("c")]), []);
...
for (let t = 100; t <= 25_000; t += 100) {
  ...
  if (target !== prior && candidateSince < 0) candidateSince = t;
...
for (let t = 1040; t < 6040; t += 1000) assert.equal(encounter.update(frame(t)), "aftermath");
...
function active(mastery: WeaponMastery) {
  ...
  if (state.phase !== "active") throw new Error("unreachable");
  return state;
}
...
for (let i = 0; i < 60; i++) glaive.advance(1500, true, "GLAIVE");
