...
// camera zoom — pixels are crisp at integer-ish zoom
...
export const zoomForWidth = (width: number): number =>
  Math.max(2, Math.min(ZOOM, Math.round((width / (TILE * 9)) * 4) / 4));
...
// restored on sleep
export const HP_REGEN_PER_DAY = 30;
...
// oxlint-disable-next-line sort-keys -- listed bottom-to-top, so the table reads as the render stack
export const DEPTH = {
  ground: 0,
  soil: 1,
  highlight: 2,
