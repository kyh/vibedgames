...
if (process.argv.includes("--cost")) {
  ...
  for (let round = 0; round < 6; round += 1) {
    for (let index = 0; index < modes.length; index += 1) {
      ...
      for (const cell of cells) {
        if (mode === "direct") {
          const built = await buildParcelGeometry(cell.plans, cell.detail, undefined, cell.lots);
      ...
      console.log(JSON.stringify({ elapsed, mode, round, vertices }));
    }
  }
  console.log(
    JSON.stringify({
  ...
  for (let run = 0; run < 3; run += 1) {
    ...
    for (let frame = 0; frame < 240; frame += 1) {
      ...
      if (state.pending === 0 && settledAt === 0) {
        settledAt = frame + 1;
      }
      states.push({ detailed: state.detailedCells, frame: frame + 1, resident: state.resident });
    }
    detailedAt =
