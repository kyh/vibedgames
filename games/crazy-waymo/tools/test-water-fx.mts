...
import { WaterFx } from "../src/fx/water-fx.ts";
import type { WaterSprayKind } from "../src/fx/water-fx.ts";
import { DRY_WATER_CONTACT } from "../src/vehicle/water-contact.ts";
import type { WaterContact } from "../src/vehicle/water-contact.ts";
...
const liveParticles = (fx: Fx): number => {
...
const dispose = (water: WaterFx): void => {
  water.mesh.geometry.dispose();
  water.mesh.material.dispose();
};

const checkWakeCadence = (check: Check): void => {
  ...
  for (const hz of [30, 60, 144]) {
    ...
    for (let i = 0; i < hz * 10; i += 1) {
      cadence.contact(1 / hz, FAST, 0, -0.7, (i / hz) * 20, 0, 20);
    }
...
const checkWakeDirection = (check: Check): void => {
  for (const speed of [-20, 20]) {
    const direction = new WaterFx(() => {
      /* empty */
    });
    direction.contact(0, STILL, 0, -0.7, 0, 0, 0);
    // retire entry ripple; retain wet state
    direction.update(1.4);
    ...
    if (indices) {
      for (let i = 0; i < direction.mesh.geometry.drawRange.count; i += 1) {
...
const checkWaterPool = (check: Check): void => {
  const fx = new Fx();
  const scene = new THREE.Scene();
  fx.addTo(scene);
  const { geometry } = fx.water.mesh;
...
export const checkWaterFx = (check: Check): void => {
  ...
  checkWakeCadence(check);
  checkWakeDirection(check);
  checkWaterPool(check);
};
