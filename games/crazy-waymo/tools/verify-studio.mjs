...
const review = () => {
  ...
  report.worldRevision = evaluate(async () => {
    const world = await import("/src/world/world-bin.ts");
    return world.WORLD_REV;
  });
  ...
  evaluate(() => {
    ...
    if (!taxi) {
      throw new Error("Dev-only Taxi hooks unavailable");
    }
  ...
  const driving = evaluate(async () => {
    const taxi = window.__taxi;
    if (!taxi) {
      throw new Error("Taxi not ready");
    }
    ...
    let boosted = false;
    let drifted = false;
    const hold = async (keys, ms, reached) => {
      ...
      try {
        ...
        await new Promise((resolve) => {
          const tick = () => {
            ...
            if (reached?.() || performance.now() >= until) {
              resolve();
            } else {
  ...
  const fare = evaluate(async () => {
    ...
    const stageNear = (pos) => {
      ...
      if (!hit) {
        throw new Error("Fare has no reachable road");
      }
    ...
    const until = async (predicate) => {
      ...
      await new Promise((resolve) => {
        const tick = () => {
          if (predicate() || performance.now() > deadline) {
            resolve();
          } else {
            requestAnimationFrame(tick);
          }
    ...
    if (!objective) {
      throw new Error("No passenger objective");
    }
    ...
    if (!pickedUp || !destination) {
      return { completed: false, pickedUp, score: game.state.score };
    }
  ...
  const paused = evaluate(async () => {
    ...
    // oxlint-disable-next-line promise/avoid-new -- page context has no timers/promises
    await new Promise((resolve) => {
  ...
  for (const view of [
    { name: "sunset-noon", phase: 0.25, u: 0.235, v: 0.674 },
    { name: "haight-noon", phase: 0.25, u: 0.437, v: 0.401 },
    { name: "chinatown-noon", phase: 0.25, u: 0.67, v: 0.167 },
    { name: "north-beach-golden", phase: 0.4, u: 0.675, v: 0.112 },
    { name: "dogpatch-noon", phase: 0.25, u: 0.79, v: 0.51 },
    { name: "downtown-noon", phase: 0.25, u: 0.738, v: 0.19 },
    { name: "downtown-night", phase: 0.7, u: 0.738, v: 0.19 },
  ]) {
    evaluate(async ({ u, v, phase }) => {
      ...
      await Promise.all(
        (document.querySelector("#district")?.getAnimations() ?? []).map((animation) =>
  ...
  if (report.checks.some((result) => !result.passed)) {
    process.exitCode = 1;
  }
};
