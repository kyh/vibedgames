...
if (!raw) throw new Error("Usage: node tools/prepare-sfx.mjs <original-directory> [clip-name...]");
...
for (const sound of manifest.sounds) {
  if (selected.size > 0 && !selected.has(sound.name)) continue;
  ...
  if (sound.loop) {
    ...
    frames = frames.slice(overlap, -overlap).concat(join);
  } else {
    ...
    if (first < 0 || last < first) throw new Error(`Silent generation: ${sound.name}`);
  ...
  if (rms < 0.0001) throw new Error(`Empty audio: ${sound.name}`);
  ...
  frames.forEach((frame, i) =>
    frame.forEach((value, channel) => output.writeFloatLE(value * gain, i * 8 + channel * 4)),
  );
  ...
  for (let i = 0; i < decoded.length; i += 4) {
    ...
    if (!Number.isFinite(value)) throw new Error(`Invalid PCM: ${sound.name}`);
    finalPeak = Math.max(finalPeak, Math.abs(value));
    finalSum += value * value;
  }
  if (finalPeak > 0.9) throw new Error(`Insufficient headroom: ${sound.name}`);
  const analysis = {
