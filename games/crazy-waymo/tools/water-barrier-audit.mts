...
import type { ShoreBarrier, ShoreVisualBox } from "../src/world/shoreline";
...
import { roadIntrusions, solidObb } from "./geometry-audit.mts";
import type { AuditWorld, PropInstance } from "./geometry-audit.mts";

type Check = (name: string, passed: boolean, detail?: string) => void;
interface Expected {
  readonly solid: Solid;
}
export interface WaterBarrierMatches {
...
const nearbyEntries = <T extends Expected>(
  entries: readonly T[],
): ((x: number, z: number) => T[]) => {
  ...
  return (x, z) => {
    ...
    for (const dx of [-4, 0, 4]) {
      for (const dz of [-4, 0, 4]) {
        result.push(...(buckets.get(key(x + dx, z + dz)) ?? []));
      }
    }
    return result;
  };
...
const partWithinEnvelope = (
  part: ShoreVisualBox,
  barrier: ShoreBarrier,
  frame: BarrierFrame,
): boolean => {
  const s = barrier.solid;
  const box = part.solid;
  const dx = (box.minX + box.maxX) / 2 - frame.cx;
  const dz = (box.minZ + box.maxZ) / 2 - frame.cz;
  return !(
    Math.abs(Math.sin((box.yaw ?? 0) - (s.yaw ?? 0))) > 1e-8 ||
    Math.abs(dx * frame.cos - dz * frame.sin) + (box.maxX - box.minX) / 2 > frame.hx + 1e-8 ||
    Math.abs(dx * frame.sin + dz * frame.cos) + (box.maxZ - box.minZ) / 2 > frame.hz + 1e-8 ||
...
const envelopeExtents = (parts: readonly ShoreVisualBox[], frame: BarrierFrame) => {
  let minY = Infinity;
  let maxY = -Infinity;
  let outerX = 0;
  let outerZ = 0;
  for (const part of parts) {
    ...
    outerX = Math.max(
      outerX,
      Math.abs(dx * frame.cos - dz * frame.sin) + (box.maxX - box.minX) / 2,
    );
    outerZ = Math.max(
      outerZ,
      Math.abs(dx * frame.sin + dz * frame.cos) + (box.maxZ - box.minZ) / 2,
    );
  }
  return { maxY, minY, outerX, outerZ };
};
...
const railsCoverContour = (
  parts: readonly ShoreVisualBox[],
  base: ShoreBarrier["solid"],
  s: ShoreBarrier["solid"],
): boolean => {
  let previousTop = base.maxY;
  for (const part of parts) {
    if (part.role !== "rail" && part.role !== "cap") {
      continue;
    }
    if (
...
export const shoreVisualEnvelopeMatches = (barrier: ShoreBarrier): boolean => {
  const parts = shoreBarrierVisuals(barrier);
  const s = barrier.solid;
  const frame: BarrierFrame = {
    cos: Math.cos(s.yaw ?? 0),
    cx: (s.minX + s.maxX) / 2,
    cz: (s.minZ + s.maxZ) / 2,
    hx: (s.maxX - s.minX) / 2,
    hz: (s.maxZ - s.minZ) / 2,
    sin: Math.sin(s.yaw ?? 0),
  };
  if (!parts.every((part) => partWithinEnvelope(part, barrier, frame))) {
    return false;
  }
  const extents = envelopeExtents(parts, frame);
  const base = parts.find((part) => part.role === "wall" || part.role === "curb")?.solid;
  if (
    !base ||
    !close(base.minX, s.minX) ||
    !close(base.maxX, s.maxX) ||
    !close(extents.outerX, frame.hx) ||
    !close(extents.outerZ, frame.hz) ||
    !close(extents.minY, s.minY) ||
    !close(extents.maxY, s.maxY)
  ) {
    return false;
  }
  return railsCoverContour(parts, base, s);
};
...
export const shoreAccessClear = (barriers: readonly ShoreBarrier[]): boolean => {
...
export const shoreRailJointsConnected = (barriers: readonly ShoreBarrier[]): boolean => {
  ...
  return barriers.every((barrier) => {
    if (barrier.treatment.kind === "seawall") {
      return true;
    }
    const s = barrier.solid;
...
export const hasSupportedRoadThrough = (
  network: RoadNetwork,
  hit: NearestHit,
...
export const auditWaterBarriers = (
  check: Check,
  ...
  const wallProps = new Set<PropInstance>();
  ...
  for (const [i, item] of rest.batchItems.entries()) {
    if (item.url !== null || item.raw === null) {
      continue;
    }
    const raw = rest.rawGeos[item.raw];
    const prop = props[i];
    if (!raw || !prop || raw.position.length !== 72) {
      continue;
    }
    ...
    wallProps.add(prop);
    if (raw.mat.color !== SHORE_COLORS[match.style]) {
      wrongColor += 1;
    }
