...
const REAR = 35;
const STEP = 2;
// car collision radius 1.05, plus room around its opening route
const CLEARANCE = 1.6;
...
const corridorHitsSolid = (spawn: PlayerSpawn, solid: Solid): boolean => {
  ...
  for (const axis of axes) {
    if (Math.abs(axis.direction) < 1e-8) {
      if (Math.abs(axis.origin) > axis.half) {
        return false;
      }
      continue;
    }
...
export const isPlayerSpawnSafe = (world: SpawnWorld, spawn: PlayerSpawn): boolean => {
  if (![spawn.x, spawn.z, spawn.yaw].every(Number.isFinite)) {
    return false;
  }
  ...
  for (
    ...
    if (Math.abs(x) > WORLD_HALF_X * 0.88 || Math.abs(z) > WORLD_HALF_Z * 0.88) {
      return false;
    }
    const road = world.network.nearest(x, z, 16);
    if (!road || road.dist + CLEARANCE > road.edge.half) {
      return false;
    }
    if (nearFreeway(x, z, OVERHEAD_MARGIN)) {
      return false;
    }
    if (
      world.decks.some(
  ...
  world.solids.forEachIn(
    ...
    (solid) => {
      if (corridorHitsSolid(spawn, solid)) {
        obstructed = true;
      }
    },
  );
...
export const choosePlayerSpawn = (
  world: SpawnWorld,
