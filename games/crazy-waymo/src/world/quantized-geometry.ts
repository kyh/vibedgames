...
export function qPos(a: Float32Array): QPos {
  let minX = Infinity,
    minY = Infinity,
    minZ = Infinity;
  let maxX = -Infinity,
  ...
  if (a.length === 0) minX = minY = minZ = maxX = maxY = maxZ = 0;
...
export function qNor(a: Float32Array): Int8Array {
  const q = new Int8Array(a.length);
  for (let i = 0; i < a.length; i++) q[i] = Math.round((a[i] ?? 0) * 127);
  return q;
}

export function qCol(a: Float32Array): Uint8Array {
...
export function allZero(a: Float32Array): boolean {
  for (let i = 0; i < a.length; i++) if (a[i] !== 0) return false;
  return true;
}

export function qUv(a: Float32Array): QUv {
  let minU = Infinity,
    minV = Infinity,
    maxU = -Infinity,
    maxV = -Infinity;
  ...
  if (a.length === 0) minU = minV = maxU = maxV = 0;
...
export function dqUv(p: QUv): Float32Array {
...
function float32Of(attr: THREE.BufferAttribute | THREE.InterleavedBufferAttribute): Float32Array {
...
export function packGeometry(geo: THREE.BufferGeometry): PackedGeometry {
  const pos = geo.getAttribute("position");
  if (!pos) throw new Error("packGeometry: geometry has no position attribute");
...
export function buildPackedGeometry(p: PackedGeometry): THREE.BufferGeometry {
  ...
  if (p.uv) geo.setAttribute("uv", new THREE.BufferAttribute(dqUv(p.uv), 2));
  if (p.col) geo.setAttribute("color", new THREE.BufferAttribute(p.col, 3, true));
...
export function constantColorAttribute(count: number, color: THREE.Color): THREE.BufferAttribute {
...
export function seatPackedMesh(mesh: THREE.Object3D, p: QPos): void {
...
export function normalizedScale(attr: THREE.BufferAttribute): number {
  ...
  if (a instanceof Uint32Array) return 1 / 4294967295;
  if (a instanceof Int32Array) return 1 / 2147483647;
  return 1;
}
