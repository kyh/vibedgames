...
export const waterIntersectsReservation = (body: WaterBody, box: Box): boolean => {
...
export const carveWaterReservations = (
  ...
  const visit = (box: Solid, candidates: readonly WaterBody[]): void => {
    ...
    if (overlapping.some((body) => points.every((p) => waterBodyContains(body, p.x, p.z)))) {
      return;
    }
    const width = box.maxX - box.minX;
    const depth = box.maxZ - box.minZ;
    if (Math.max(width, depth) <= EDGE_RESOLUTION) {
      return;
    }
    if (width >= depth) {
