...
export const stowRadius = (x: number, z: number): number =>
  Math.hypot((x - CENTER_X) / GGP_LAKE.ru, (z - CENTER_Z) / GGP_LAKE.rv);

/** Whole-footprint exclusion shared by rendered park tiles and their terraces. */
export const stowBasinOverlapsBox = (
  minX: number,
  maxX: number,
  minZ: number,
  maxZ: number,
...
export const inLake = (x: number, z: number): boolean => stowRadius(x, z) < 1;

export const stowWaterHeightAt = (x: number, z: number): number | null =>
  inLake(x, z) ? STOW_WATER_Y : null;
...
export const stowBasinHeight = (x: number, z: number, naturalHeight: number): number => {
