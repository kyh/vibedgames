...
export const surfaceDeckHeight = (deck: SurfaceDeck, z: number): number => {
  if (deck.y2 === undefined || deck.maxZ <= deck.minZ) {
    return deck.y;
  }
...
export const surfaceDeckPhysics = (decks: readonly SurfaceDeck[]): Float32Array => {
  ...
  for (let ix = 0; ix + 1 < xs.length; ix += 1) {
    ...
    for (let iz = 0; iz + 1 < zs.length; iz += 1) {
      ...
      const by = surfaceDeckHeight(deck, bz);
