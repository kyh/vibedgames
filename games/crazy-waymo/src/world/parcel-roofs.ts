...
const convexEdges = (p: ParcelPlan): readonly RingEdge[] | null => {
  ...
  for (let i = 0; i < p.n; i += 1) {
    ...
    edges.push({ length, tx: dx / length, tz: dz / length, x, z });
  }
  for (const edge of edges) {
    for (const vertex of edges) {
      if (edge.tx * (vertex.z - edge.z) - edge.tz * (vertex.x - edge.x) < -0.001) {
        return null;
      }
    }
  }
...
const cornerEdgePair = (
  p: ParcelPlan,
  edges: readonly RingEdge[],
  corner: number,
): readonly [RingEdge, RingEdge] | null => {
...
const mansardInset = (p: ParcelPlan, edges: readonly RingEdge[]): Float32Array => {
  ...
  return p.ring.map((value, index) => value * 0.74 + (index % 2 === 0 ? centerX : centerZ) * 0.26);
};

/** Rare historic corner roofs. Strict convexity makes every generated face stay in the parcel. */
export const roofVariantOf = (p: ParcelPlan): RoofVariant | null => {
  if (
    p.kind !== "rowhouse" ||
  ...
  // seed is a uint32 hash; the low nibble already drives other rolls
  const roll = Math.floor(p.seed / 16) % 19;
  if (roll > 1) {
    return null;
  }
  const edges = convexEdges(p);
  if (!edges) {
    return null;
  }
  const front = edges[p.front];
  if (!front || front.length < 3 || p.blind[p.front] === 1) {
    return null;
  }
  const candidates = [p.front, (p.front + 1) % p.n];
  for (const corner of candidates) {
    const pair = cornerEdgePair(p, edges, corner);
    if (!pair) {
      continue;
    }
    const [a, b] = pair;
    ...
    if (roll === 0) {
      return { cx, cz, kind: "turret", radius, rise: 1.2 };
    }
    return { inset: mansardInset(p, edges), kind: "mansard", rise: 0.85 };
  }
  return null;
};
