...
export interface StreetKitPart {
...
interface Shelter {
  ...
  readonly kit: readonly StreetKitPart[];
}

const materials = {
  charcoal: new THREE.MeshStandardMaterial({ color: 0x30_38_3d, metalness: 0.15, roughness: 0.48 }),
  cream: new THREE.MeshStandardMaterial({ color: 0xee_e4_d0, roughness: 0.72 }),
  ...
  letter: new THREE.MeshStandardMaterial({ color: 0xf5_f3_e7, roughness: 0.65 }),
  red: new THREE.MeshStandardMaterial({ color: 0xbd_25_33, metalness: 0.08, roughness: 0.3 }),
  steel: new THREE.MeshStandardMaterial({ color: 0x9c_a9_ac, metalness: 0.42, roughness: 0.4 }),
...
const canopyHeight = (x: number): number => 2.48 + 0.24 * Math.cos(((x + 1.12) * Math.PI) / 1.18);

const canopyGeometry = (bottom: number, top: number): THREE.BufferGeometry => {
...
interface ShelterDraw {
  readonly add: (name: string, key: MaterialKey, geo: THREE.BufferGeometry) => void;
  readonly box: (
    name: string,
    key: MaterialKey,
    at: Vec3,
...
/** Raised vector lettering: one merged box per horizontal stroke of the 5x7 font. */
const drawText = (d: ShelterDraw, value: string, at: Vec3, pixel: number, yaw = 0): void => {
  let cursor = 0;
  for (const char of value) {
    ...
    for (let row = 0; row < rows.length; row += 1) {
      ...
      for (let column = 0; column < cells.length; column += 1) {
        ...
        let stroke = column + 1;
        while (cells[stroke] === "1") {
          stroke += 1;
        }
        const px = cursor + (column + (stroke - column) / 2) * pixel;
        d.box(
          `letter-${char}`,
          "letter",
          [at[0] + px * Math.cos(yaw), at[1] - row * pixel, at[2] - px * Math.sin(yaw)],
          [(stroke - column) * pixel, pixel * 0.91, 0.012],
          0,
          yaw,
        );
        column = stroke - 1;
...
const drawCanopy = (d: ShelterDraw): void => {
  d.add("canopy", "red", canopyGeometry(-0.065, 0.075));
  // A separate dark soffit gives the curled enamel fascia a readable lip.
  d.add("soffit", "charcoal", canopyGeometry(-0.087, -0.067));
  for (let i = 0; i < 15; i += 1) {
    const x = -1.99 + (3.98 * i) / 14;
    d.box("soffit-rib", "steel", [x, canopyHeight(x) - 0.115, 0], [0.035, 0.045, 1.76]);
  }
  for (const z of [-0.34, 0.34]) {
    ...
    d.rod("canopy-seam", "red", points, 0.008, 48);
  }
};

const drawFrame = (d: ShelterDraw): void => {
  for (const x of [-1.91, 1.91]) {
    const height = canopyHeight(x) - 0.09;
    for (const z of [-0.7, 0.69]) {
      d.box("post", "steel", [x, height / 2, z], [0.1, height, 0.1], 0.012);
      d.box("foot-plates", "steel", [x, 0.035, z], [0.24, 0.07, 0.23], 0.012);
      d.box("foot-collar", "steel", [x, 0.11, z], [0.15, 0.17, 0.15], 0.012);
    ...
    d.box("side-rail", "steel", [x, 2.21, -0.01], [0.1, 0.085, 1.46], 0.01);
  }
  d.box("rear-top-rail", "steel", [0, 2.21, -0.72], [3.86, 0.09, 0.1], 0.012);
  d.box("rear-bottom-rail", "steel", [0, 0.25, -0.72], [3.86, 0.065, 0.09], 0.01);
  for (const x of [-0.645, 0.645]) {
    d.box("rear-mullion", "steel", [x, 1.24, -0.72], [0.055, 2.02, 0.055], 0.008);
  }
};

const drawGlazing = (d: ShelterDraw): void => {
  for (let i = 0; i < 3; i += 1) {
    d.box("glass", "glass", [-1.28 + i * 1.28, 1.245, -0.725], [1.2, 1.84, 0.028]);
  }
  for (const x of [-1.91, 1.91]) {
    d.box("glass-return", "glass", [x, 1.245, -0.025], [0.028, 1.84, 1.3]);
  }
  for (const x of [-1.86, -0.68, -0.61, 0.61, 0.68, 1.86]) {
    for (const y of [0.43, 2.04]) {
      d.box("glass-clamp", "steel", [x, y, -0.695], [0.085, 0.035, 0.045], 0.006);
    }
  }
};

const drawBench = (d: ShelterDraw): void => {
  ...
  for (let i = 0; i < 7; i += 1) {
    ...
    d.box("bench-seat-slat", "charcoal", [benchX, 0.63 + i * 0.004, z], [3.42, 0.045, 0.057]);
  }
  for (let i = 0; i < 7; i += 1) {
    const y = 0.735 + i * 0.075;
    const z = -0.39 - (y - 0.735) * 0.18;
    d.box("bench-back-slat", "charcoal", [benchX, y, z], [3.42, 0.059, 0.048]);
  }
  ...
  for (const x of [-1.33, 1.14]) {
    d.box("bench-leg", "steel", [x, 0.33, -0.23], [0.08, 0.56, 0.1], 0.01);
    d.box("bench-foot", "steel", [x, 0.055, -0.23], [0.19, 0.045, 0.34], 0.01);
  }
...
const drawSignage = (d: ShelterDraw): void => {
  d.box("front-sign", "steel", [1.1, 2.465, 0.883], [0.88, 0.28, 0.045], 0.026);
  d.box("front-sign-enamel", "red", [1.1, 2.465, 0.91], [0.82, 0.235, 0.026], 0.021);
  drawText(d, "MUNI", [0.839, 2.527, 0.933], 0.022);
  ...
  const panelX = 1.976;
  const yaw = Math.PI / 2;
  d.box("route-panel-frame", "steel", [panelX, 1.25, 0.14], [0.79, 1.92, 0.1], 0.035, yaw);
  d.box("route-panel", "cream", [panelX + 0.056, 1.25, 0.14], [0.69, 1.79, 0.02], 0.022, yaw);
  d.box("route-header", "red", [panelX + 0.071, 1.94, 0.14], [0.675, 0.36, 0.017], 0.015, yaw);
  drawText(d, "MUNI", [panelX + 0.086, 2.008, 0.389], 0.021, yaw);
  d.box("route-footer", "red", [panelX + 0.072, 0.5, 0.14], [0.675, 0.28, 0.017], 0.012, yaw);
  drawText(d, "SF", [panelX + 0.088, 0.558, 0.363], 0.023, yaw);
  d.add(
    "route-badge",
    "red",
  ...
  drawText(d, "7", [panelX + 0.09, 1.653, 0.381], 0.012, yaw);
  d.box("route-line", "red", [panelX + 0.075, 1.12, 0.352], [0.017, 0.76, 0.018]);
  for (let i = 0; i < 6; i += 1) {
...
const buildShelter = (): Shelter => {
  ...
  const add = (name: string, key: MaterialKey, geo: THREE.BufferGeometry): void => {
    pieces.push({ geo: geo.index ? geo.toNonIndexed() : geo, key, mat: materials[key], name });
  };
  ...
  const d: ShelterDraw = { add, box, rod };
  drawCanopy(d);
  drawFrame(d);
  drawGlazing(d);
  drawBench(d);
  ...
  for (const [key, geos] of grouped) {
    ...
    if (!geo) {
      throw new Error("Muni shelter material geometry is incompatible");
    }
...
const getShelter = (): Shelter => {
  shelter ??= buildShelter();
  return shelter;
};

export const getMuniShelterKit = (): readonly StreetKitPart[] => getShelter().kit;

/** Named, shared geometry for image-to-threejs review; placements use the kit. */
export const createMuniShelterModel = (
