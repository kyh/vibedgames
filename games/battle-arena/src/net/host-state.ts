...
import { applySnapshot, encodeWorld, type Snapshot } from "./snapshot";

const ONLINE_SEED = 0xbada55;
export type HeroPick = { champId: string; name: string };
export type OnlineSeat = { slot: number; team: string };

export function humanRoster(world: World) {
  const picks: Record<string, HeroPick> = {};
  const seats: Record<string, OnlineSeat> = {};
  for (const unit of world.units.values()) {
    if (unit.kind !== "hero" || unit.isBot) continue;
...
export function reconcileHostHeroes(
  ...
  for (const unit of world.units.values()) {
    if (unit.kind !== "hero" || unit.isBot) continue;
    if (!players[unit.ownerId]) world.units.delete(unit.id);
    else {
      seats[unit.ownerId] = { slot: unit.slot, team: unit.team };
  ...
  for (const ownerId of Object.keys(players)) {
    if (world.units.has(`h-${ownerId}`)) continue;
