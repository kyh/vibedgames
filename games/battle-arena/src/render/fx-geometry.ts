...
export interface CrystalOpts {
  seed?: number;
  // facets around the prism — 5..8 read best
  sides?: number;
  // tip radius as a fraction of the base
  taper?: number;
  // how far facets are pushed off a clean prism
  roughness?: number;
 ...
 */
export const createCrystalGeometry = ({
  seed = 1,
  sides = 6,
  taper = 0.13,
  roughness = 0.28,
  ...
  const axisOffset = (t: number) => bend * 0.5 * t ** 1.6;
  ...
  const rings: Pt[][] = RING_HEIGHTS.map((t, ringIndex) => {
    const profile = tipRadius + (1 - tipRadius) * (1 - t) ** 1.15;
 ...
 */
export const createShardGeometry = (seed = 5, sides = 5): THREE.BufferGeometry =>
...
const lattice = (ix: number, iy: number, iz: number, seed: number): number =>
  hash11(ix * 127.1 + iy * 311.7 + iz * 74.7 + seed * 19.19);
 ...
 */
const valueNoise3 = (x: number, y: number, z: number, seed: number): number => {
...
const fbmValue = (x: number, y: number, z: number, seed: number, octaves: number): number => {
  let value = 0;
  let amplitude = 0.5;
  let frequency = 1;
...
export interface RockOpts {
  seed?: number;
  // icosphere subdivisions, 0..3
  detail?: number;
  // low-frequency deformation, × radius
  lumpiness?: number;
  noiseScale?: number;
  // high-frequency chipping
  roughness?: number;
  // planar fracture faces sliced off it
  cuts?: number;
  cutDepth?: number;
  craters?: number;
  craterDepth?: number;
...
export const createRockGeometry = ({
  ...
  for (let v = 0; v < posAttr.count; v += 1) {
    ...
    for (const bowl of bowls) {
      ...
      radius += bowl.depth * 0.5 * smoothstep(0.72, 1, q) * (1 - smoothstep(1, 1.4, q));
    }
