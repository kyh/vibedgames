...
const feedSigil = (u: Unit | undefined): string =>
  u && u.kind === "hero" && u.champId
...
/** Typed div lookup inside a plate this HUD just built from its own markup. */
const queryDiv = (root: HTMLElement, sel: string): HTMLDivElement => {
  const el = root.querySelector(sel);
  return el instanceof HTMLDivElement ? el : document.createElement("div");
};
...
/** A level-locked tile: no sweep, "Lv4" on the ult, ready-flash armed off. */
const showLockedTile = (el: AbilityEl, key: AbilityKey): void => {
...
const abilityTileClass = (key: AbilityKey, util: boolean): string => {
  if (key === "R") {
    return "ba-abil ult";
  }
  return util ? "ba-abil util" : "ba-abil";
};

/** HP bar gradient tier. */
...
/** Cooldown label: tenths under 10s, whole seconds above, empty when ready. */
const cdLabel = (cdLeft: number): string => {
  if (cdLeft <= 0) {
    return "";
  }
...
const plateNameColor = (u: Unit, isLocal: boolean): string => {
  if (u.kind === "creep") {
    return "#b8c0d0";
  }
  return isLocal ? hex(LOCAL_COLOR) : hex(teamColor(u.team));
};

const plateFillColor = (u: Unit, myTeam: string): string => {
  if (u.kind === "creep") {
    return "#c8a0a0";
  }
  return u.team === myTeam ? "#5dd66b" : "#ff5a52";
};

/** Goal banner opacity: solid for 8s, faded out by 10s. */
...
const saveBestKills = (kills: number): void => {
  try {
...
export class Hud {
  ...
  constructor(view: View, fx: Fx, shop: ShopCallbacks) {
    this.view = view;
    this.fx = fx;
    this.shop = shop;
    this.root = byId("hud");
  ...
  private build(): void {
    ...
    for (const key of ALL_ABILITY_KEYS) {
      ...
      wrap.className = abilityTileClass(key, util);
  ...
  private hitsTopBand(x: number, y: number): boolean {
    this.topBand ??= this.topEl.getBoundingClientRect();
  ...
  private updatePlates(w: World, me: Unit): void {
    ...
    for (const u of w.units.values()) {
      ...
      if (u.kind === "creep" && (u.x - me.x) ** 2 + (u.y - me.y) ** 2 > 22 * 22) {
        continue;
      }
      seen.add(u.id);
      ...
      if (!plate) {
        ...
        const isLocal = u.id === me.id;
        const col = plateNameColor(u, isLocal);
        const name = u.kind === "creep" ? "" : u.name;
        wrap.innerHTML = `<div class="ba-pname" style="color:${col}">${name}</div><div class="ba-php"><div class="ba-phpfill" style="background:${plateFillColor(u, me.team)}"></div></div>`;
        byId("ba-plates").append(wrap);
        plate = {
          fill: queryDiv(wrap, ".ba-phpfill"),
          name: queryDiv(wrap, ".ba-pname"),
          wrap,
        };
  ...
  private updateBuffs(w: World, me: Unit): void {
    const chips = this.collectBuffChips(w, me);
  ...
  /** Live statuses collapsed to one chip per kind (longest `until` wins), plus
   *  the synthetic empower chip. Silence etc. — no icon, not hex — gets none. */
  private collectBuffChips(w: World, me: Unit): { kind: string; until: number }[] {
  ...
  private drainFeed(w: World): void {
    for (const k of this.fx.feed.splice(0)) {
      ...
      while (this.feedEl.childElementCount > cap) {
        this.feedEl.firstElementChild?.remove();
      }
    }
    for (const t of this.fx.toasts.splice(0)) {
  ...
  private updateEnd(w: World, me: Unit): void {
    ...
    if (won && winner) {
      ...
      const wx = winner.x;
      const wy = winner.y;
      for (const [i, c] of [0xff_d2_4a, 0x6b_ff_8e, 0x9f_d0_ff].entries()) {
