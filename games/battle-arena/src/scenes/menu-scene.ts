// Pre-match lobby. The six champions are shown as 3D models in a row (rendered
// by MenuStage); this DOM overlay sits on top with the logo, the selected
// champion's details, clickable name chips, name/room inputs and the play
// buttons. The overlay is click-through except on its controls, so clicks in
// the middle reach the 3D row for selection.
...
export class Menu {
  ...
  private build(): void {
    ...
    const chips = CHAMPIONS.map(
      (c) => `<button class="ba-chip" data-id="${c.id}" style="--accent:${hex(c.tint)}">${c.name}</button>`,
    ).join("");
    ...
    this.el.innerHTML = `
      ...
      <div class="ba-bottom">
        ...
        <div class="ba-row2">
          <input id="ba-name" maxlength="14" placeholder="Your name" value="${params.get("name") ?? ""}" />
          <input id="ba-room" maxlength="12" placeholder="Room code (optional)" value="${params.get("room") ?? ""}" />
        </div>
  ...
  setSelected(id: string): void {
    ...
    if (c && info) {
      ...
      info.innerHTML = `<span class="ba-i-name">${c.name}</span><span class="ba-i-title">${c.title}</span>
        <span class="ba-i-role">${c.role} · ${c.primary.toUpperCase()}</span><span class="ba-i-ab">${ab}</span>`;
    }
  }
...
.ba-info{min-height:64px;display:flex;flex-direction:column;align-items:center;gap:2px}
...
.ba-i-ab{font-size:12px;opacity:.6;margin-top:2px}
...
.ba-chip{pointer-events:auto;background:rgba(20,26,42,.85);border:2px solid rgba(255,255,255,.12);border-radius:10px;padding:8px 14px;color:#fff;cursor:pointer;font:800 14px ui-monospace,monospace;transition:transform .1s,border-color .1s,box-shadow .1s}
.ba-chip:hover{transform:translateY(-2px)}
.ba-chip.sel{border-color:var(--accent);color:var(--accent);box-shadow:0 0 20px -6px var(--accent)}
