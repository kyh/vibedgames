...
- **A player who is trying: `vg pilot`.** `vg pilot --url <url> --goal "<what wins, what kills, which way is progress>" --json` hands the controls to a decision model that reads `__GAME_DIAGNOSTICS__` several times a second (through the vibedgames API; nothing to configure). Its `timeline` and `decisions.meanConfidence` are onboarding and readability evidence: a model told the rules that still can't find the score, or can't prefer a direction, is a finding about what the game shows. If the game has no `window.__GAME_PILOT__` manifest, file that as a backlog item for the engineer — the pilot falls back to WASD + Space, which is only right by accident.
