...
import { readDirective } from "./state.ts";
import type { AgentState, Blackboard, Phase } from "./state.ts";

export { ROLES } from "./agents.ts";
export type { Role, RoleName } from "./agents.ts";

const readNext = (bb: Blackboard): NextAssignment => {
  try {
    const parsed = asJsonObject(parseJson(readFileSync(bb.next, "utf-8")));
    if (parsed) {
      const { role } = parsed;
      if (role === "designer" || role === "engineer" || role === "artist" || role === "qa") {
        return { role, task: asString(parsed.task) ?? "", type: asString(parsed.type) };
      }
    }
  } catch {
  ...
  return {
    role: "engineer",
    task: "next.json was missing or invalid — pick the highest-priority unfinished item from .vgfactory/backlog.json and implement it.",
  };
};
/** Which subagent runs a given phase. `work` is routed dynamically. */
export const roleForPhase = (phase: Phase, bb: Blackboard): RoleName => {
  switch (phase) {
    case "spec": {
      return "designer";
    }
...
const phaseTask = (phase: Phase, state: AgentState, bb: Blackboard): string => {
  const { slug } = state;
  // Tell subagents to read the operator's brief/reference when one was given.
  const ctx = existsSync(bb.context)
    ? "First read ./.vgfactory/context.md for the operator's brief / reference, and honor it. "
    : "";
  switch (phase) {
    case "spec": {
      if (state.existingProject) {
        return `${ctx}The game directory ALREADY contains a project — build UPON it, don't start over. Read the existing source (package.json, src/, index.html, any README) to understand what it is and which engine/stack it uses. Then write ./.vgfactory/spec.md describing the current game honestly and a concrete plan to finish/evolve it, and record the detected engine as a line exactly like \`engine: phaser\`, \`engine: threejs\`, or \`engine: other\`. Slug is "${slug}". Append a one-line summary to journal.md.`;
      }
      return `${ctx}Design the game from this seed idea: "${state.idea}".\nThe game slug is "${slug}". Produce ./.vgfactory/spec.md per your role instructions, then append a one-line summary to journal.md.`;
    }
    case "scaffold": {
      if (state.existingProject) {
        return `${ctx}A project already exists in the current directory — do NOT run \`vg new\` (it would overwrite the existing code). Instead make it deployable: \`npm install\`, ensure \`npm run build\` and \`npm run typecheck\` succeed (add a typecheck script if missing), make sure a \`vibedgames.json\` with {"slug":"${slug}"} exists (\`vg deploy\` needs it), and ensure \`.vgfactory/\` is git-ignored (skip if the governing repo's .gitignore already covers it — don't add a redundant per-game one). Fix any build breakage. Append what you did to journal.md.`;
      }
      return `${ctx}Read ./.vgfactory/spec.md for the engine choice. The current directory IS the game project root (it already contains the ./.vgfactory folder). Scaffold the engine here by running \`vg new ${slug} --engine <phaser|threejs from spec> --here --force\`, then \`npm install\`. Confirm \`npm run build\` works on the fresh template. Append what you did to journal.md.`;
    }
    case "assets": {
      ...
      return `${ctx}Read ./.vgfactory/spec.md and produce the art for "${slug}" per your role instructions. ${note} Record frame sizes and file paths in journal.md.`;
    }
    case "build": {
      const note = state.existingProject
      ...
      return `${ctx}Read ./.vgfactory/spec.md. Get the core gameplay loop for "${slug}" working using the assets the artist generated (check journal.md for paths and frame sizes). ${note} Verify with typecheck + build.`;
    }
    case "playtest": {
      ...
      const suiteNote = unchanged
        ...
        : "";
      return `Playtest the current build of "${slug}" per your role instructions and record findings in ./.vgfactory/playtest.md and backlog.json.${suiteNote}`;
    }
    case "ship": {
...
export const buildTask = (phase: Phase, state: AgentState, bb: Blackboard): string => {
