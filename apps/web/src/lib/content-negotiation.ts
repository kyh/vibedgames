...
const parseQ = (params: string[]): number => {
  for (const param of params) {
    ...
    // An empty or unparseable q is treated as absent, i.e. q=1.
    const q = raw === "" ? Number.NaN : Number(raw);
    if (Number.isNaN(q)) {
      return 1;
    }
...
const specificityOf = (type: string, subtype: string): number => {
  if (type === "*") {
    return 0;
  }
  if (subtype === "*") {
...
export const parseAccept = (header: string | null | undefined): AcceptEntry[] => {
  if (!header) {
    return [];
  }
...
const scoreFor = (candidate: string, entries: AcceptEntry[]) => {
...
export const negotiate = (
  ...
  if (entries.length === 0) {
    return { kind: "default", type: fallback };
  }
  ...
  for (const candidate of produces) {
    ...
    if (
      ...
      winner = { q: score.q, specificity: score.specificity, type: candidate };
    }
  }
...
export const prefersMarkdown = (request: Request): boolean => {
