...
- The tilemap editor is a browser UI (`asset-tilemap-editor.mjs --edit`) rather than a Tkinter window: same map format and keys, nothing to install. It serves one page over loopback behind a per-run token, and refuses to read or write outside the working directory.
