...
const declaredNames = (argsDef: Record<string, ArgSpec>): Set<string> => {
  ...
  for (const [name, spec] of Object.entries(argsDef)) {
    ...
    names.add(name.replaceAll(/[A-Z]/gu, (c) => `-${c.toLowerCase()}`));
    const { alias } = spec;
 ...
 * left alone.
 */
export const unknownFlags = (rawArgs: string[], argsDef: Record<string, ArgSpec>): string[] => {
  ...
  for (const raw of rawArgs) {
    ...
    if (negated !== null && known.has(negated)) {
      continue;
    }
    unknown.push(`--${flag}`);
  }
...
export const assertKnownFlags = (rawArgs: string[], argsDef: Record<string, ArgSpec>): void => {
