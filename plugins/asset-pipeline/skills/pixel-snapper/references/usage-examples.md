...
```bash
node $SKILLS/pixel-snapper/scripts/pixel-snapper.mjs \
  input.png \
  output.png
```
...
```bash
chmod +x $SKILLS/pixel-snapper/scripts/pixel-snapper.mjs

$SKILLS/pixel-snapper/scripts/pixel-snapper.mjs \
...
for f in "$OUT_DIR"/*-snapped.png; do
  name=$(basename "$f" .png)
  node $SKILLS/pixel-snapper/scripts/image-util.mjs upscale \
...
- **Input dims**: `node $SKILLS/pixel-snapper/scripts/image-util.mjs size input.png`.
...
- Input is a photograph or continuous-tone illustration → use `node $SKILLS/pixel-snapper/scripts/image-util.mjs resize in.png out.png --size WxH` (Lanczos).
