...
RES_BG=$(vg media run bria/background/remove --image_url "$URL_RAW" --json)
URL_CLEAN=$(echo "$RES_BG" | jq -r '.image.url')

# Step 2: image-to-3D
SUBMIT=$(vg media run hunyuan-3d/v3.1/pro/image-to-3d \
 --image_url "$URL_CLEAN" \
 --async \
 --json)
...
vg media status hunyuan-3d/v3.1/pro/image-to-3d "$REQ" \
