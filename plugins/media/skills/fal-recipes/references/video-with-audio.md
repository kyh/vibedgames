...
vg media run fal-ai/mmaudio-v2 \
 --video_url "$URL_VIDEO" \
...
vg media run fal-ai/ffmpeg-api/merge-audio-video \
 --video_url "$URL_VIDEO" \
 --audio_url "$URL_AUDIO" \
 --download "./outputs/with-audio/{request_id}_{index}.{ext}" \
 --json
...
MERGE_RESULT=$(vg media run fal-ai/ffmpeg-api/merge-audio-video \
 --video_url "$URL_VIDEO" \
 --audio_url "$URL_AUDIO" \
 --json)
...
vg media run fal-ai/workflow-utilities/auto-subtitle \
...
```bash
vg media schema fal-ai/mmaudio-v2 --json
vg media schema fal-ai/ffmpeg-api/merge-audio-video --json
vg media schema fal-ai/workflow-utilities/auto-subtitle --json
```
