...
async function init() {
  ...
  const player = await loadModel("assets/Character_Male_1.gltf", playerFallback, playerMat);
...
class ModelCache {
  ...
  clone(path) {
    ...
    return hasAnimations ? SkeletonUtils.clone(cached.scene) : cached.scene.clone();
  }
