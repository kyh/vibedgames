...
import { constants as zlibConstants, inflateSync as inflateSync2 } from "node:zlib";
...
function inspectAseprite(path, bytes, options = {}) {
  ...
  if (decodeCels) {
    for (const fr of framesOut) {
      for (const chunk2 of fr.chunks) {
        if (chunk2.type !== "cel") continue;
        const cel = chunk2.data;
