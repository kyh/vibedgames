...
1. Routed endpoints (see [model-catalog](../../model-catalog/SKILL.md)):
...
vg generate docs "commercial product image generation" --json
```
...
vg generate upload ./logo.png --json
```
...
1. Upload the product image with `vg generate upload`.
...
4. Run async, then download from `vg generate status`.
