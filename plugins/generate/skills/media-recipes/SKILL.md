...
name: media-recipes
description: >
  Use-case-driven multi-step pipelines with `vg generate`. Trigger when the user asks
  ...
  call sequence, and quality checks. Routing: asset phase (workflow); runs through `generate`; use `media-workflow` instead for free-form pipelines.
---
...
A recipe is a use-case-driven pipeline. It tells you the inputs to collect, the vg generate calls to chain, and the quality bar to verify before returning.

Recipes use `vg generate` for execution and `model-catalog` for endpoint defaults. They differ from `media-workflow`:
...
- For utility endpoints (resize, composite, audio merge, subtitle, etc.): search `vg generate models --category` for the relevant modality.
