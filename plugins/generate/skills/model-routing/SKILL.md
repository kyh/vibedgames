...
description: >
  ...
  has not named a specific model. Routing: meta; supplies default endpoint IDs to `character-design`, `cinematography`, `storytelling`, `pixel-art`, `regenerate-3d`, `media-workflow`.
---

# Model routing

Use these endpoint defaults when a domain skill needs a model. These choices
come from project-specific guidance. Still run `vg generate schema <endpoint_id>
--json` before execution and `vg generate pricing <endpoint_id> --json` when cost
matters.

Endpoint-first rule:

1. Pick the endpoint ID from this skill.
2. Verify it with `vg generate models --endpoint_id <endpoint_id> --json`.
3. Inspect it with `vg generate schema <endpoint_id> --json`.
4. Check `vg generate pricing <endpoint_id> --json` when cost matters.
