...
function assertBuildAccess({ build, userId }: { build: GameBuildRow; userId: string }) {
...
export const localGameRouter = createTRPCRouter({
  listBuilds: protectedProcedure.input(listBuildsInput).query(async ({ ctx, input }) => {
    const limit = input.limit ?? DEFAULT_BUILD_LIMIT;

    const builds = await ctx.db.query.gameBuild.findMany({
      where: (builds, { eq, and, lt }) =>
        input.cursor
          ? and(eq(builds.userId, ctx.session.user.id), lt(builds.id, input.cursor))
  ...
  getBuild: protectedProcedure.input(getBuildInput).query(async ({ ctx, input }) => {
    const build =
  ...
  createBuild: protectedProcedure.input(createBuildInput).mutation(async ({ ctx, input }) => {
  ...
  updateBuild: protectedProcedure.input(updateBuildInput).mutation(async ({ ctx, input }) => {
    const build = await ctx.db.query.gameBuild.findFirst({
      where: (builds, { eq }) => eq(builds.id, input.buildId),
    });
  ...
  deleteBuild: protectedProcedure.input(deleteBuildInput).mutation(async ({ ctx, input }) => {
    const build = await ctx.db.query.gameBuild.findFirst({
      where: (builds, { eq }) => eq(builds.id, input.buildId),
    });
...
export async function persistFiles({ db, buildId, files }: PersistFilesParams) {
  ...
  await db.update(gameBuild).set({ updatedAt: new Date() }).where(eq(gameBuild.id, buildId));
}
